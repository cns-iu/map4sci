...
function createPopupHTML(description: Record<string, any>, content: PopupContent): string {
  let html: string = '';
...
const blankStyle: exported.Style = {
  'version': 8,
  'name': 'Blank',
  'center': [
    0,
...
class ZmltMap {
  ...
  hoverEdgeID: number | string | undefined;
  hoverNodeID: number | string | undefined;
  ...
  getZoomIndex(zoomLookup: Record<string, any>[]): number  {
  ...
  addNodes(): void {
    ...
    const nodeLabelsLayer: exported.Layer | any = {
      'id': 'node_labels',
      'type': 'symbol',
      'source': 'nodes_source',
      'layout': {
    ...
    const nodesLayer: exported.Layer | any = {
      'id': 'nodes',
      'type': 'circle',
      'source': 'nodes_source',
      'layout': {},
  ...
  addEdgeHover(): void {
    const map: exported.Map = this.map;
    map.on('mousemove', 'edges', (e: any) => {
  ...
  addPopupOnClick(layer: string, content: PopupContent): void {
    ...
    map.on('click', layer, function (e: any) {
...
export class MapComponent {
  ...
  style = blankStyle;
  map = new exported.Map;

  // Outputs
  @Output() nodeClick = new EventEmitter<exported.MapMouseEvent>();
  @Output() edgeClick = new EventEmitter<exported.MapMouseEvent>();
  ...
  nodeClicked(event: exported.MapMouseEvent): void {
    this.nodeClick.emit(event);
  }

  edgeClicked(event: exported.MapMouseEvent): void {
  ...
  // config: Configuration = {
  //   // Sources is the only required property for the config object.
  //   sources: {
  //     nodes: 'nodes.geojson',
  //     edges: 'edges.geojson',
  //     clusters: 'cluster.geojson',
